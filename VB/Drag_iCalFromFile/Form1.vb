...
Namespace Drag_iCalFromFile
	Partial Public Class Form1
		...
		Private Sub Form1_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
			...
			Me.resourcesTableAdapter.Fill(Me.appointmentsDataSet.Resources)
		...
		Private Sub OnApptChangedInsertedDeleted(ByVal sender As Object, ByVal e As PersistentObjectsEventArgs) Handles schedulerStorage1.AppointmentsChanged, schedulerStorage1.AppointmentsInserted, schedulerStorage1.AppointmentsDeleted
			Me.appointmentsTableAdapter.Update(Me.appointmentsDataSet)
			Me.appointmentsDataSet.AcceptChanges()
		...
		Private Sub schedulerControl1_DragDrop(ByVal sender As Object, ByVal e As DragEventArgs) Handles schedulerControl1.DragDrop
			If e.Data.GetDataPresent(DataFormats.FileDrop) Then
				...
				GetTargetTime(pt)
				DoFileDrop(pt, CType(e.Data.GetData(DataFormats.FileDrop), String()))
			End If
		End Sub
			...
			If files IsNot Nothing AndAlso files.Length > 0 Then
				For k As Integer = 0 To files.Length - 1
					...
					Catch ex As iCalendarEventImportException
						Dim events As VEventCollection = ex.Events
						Dim count As Integer = events.Count
						Dim message As String = String.Empty
						For i As Integer = 0 To count - 1
							Dim ev As VEvent = ex.Events(i)
							message &= String.Format("Unable to import event '{0}' started on {1:D} at {2}" & Constants.vbLf, ev.Summary.Value, ev.Start.Value.Date, ev.Start.Value.TimeOfDay)
						Next i
						MessageBox.Show(message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
					Finally
						AfterImportActions()
		...
		Private Sub importer_AppointmentImporting(ByVal sender As Object, ByVal e As AppointmentImportingEventArgs)
			...
			If (eventsCount = 1) AndAlso (targetTime <> DateTime.MinValue) Then
			   Dim doIt As DialogResult = MessageBox.Show("You can set the appointment start time to a drop destination time" & Constants.vbLf & "at " & targetTime.ToString() & "." & Constants.vbLf & "Proceed?", "Info", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
